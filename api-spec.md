# VAT-AI 스켈레톤 — DB 스키마 & API 명세 (v1, 1주차 스프린트 기준)

이 문서는 `Proposal/최종기획서_9조.pdf`(구조/워크플로우 기준)와 스켈레톤 체크리스트(1주차 범위 확정)를 근거로, 4개 트랙이 서로 다른 파일을 보지 않고도 같은 계약을 보게 하기 위한 확정 스펙이다. 여기 적힌 필드명·타입·경로·응답 형식은 임의로 바꾸지 말고, 바꿔야 하면 먼저 이 문서를 고친 뒤 코드를 맞춘다.

범위는 [VAT-AI 스켈레톤 플로우](https://claude.ai/artifact/QzqH1gaQVqoZxoiJ5whEBD)의 6단계 해피패스 + 최소 에러 처리(통일된 에러 응답, OCR 실패 상태 기록)까지다. 실패 전용 화면, 재시도, RAG, PDF 신고서 생성 등은 이번 스펙에 없다.
...
1주차는 이 문서의 응답 스키마 그대로 mock 데이터를 만들어 4화면(로그인/가입, 업로드, 목록/상세, 리포트)을 구성한다. 2주차에 이 문서의 엔드포인트로 실제 호출을 교체한다.
